...
host-incubation history in section 11), so they survive rather than being re-derived.
...
- Authenticated journeys / driving the real OIDC flow (see section 9).
...
Repo: **`github.com/infodancer/smoke`** (end state). See section 11 for the
...
state**. Integration (section 6) records *every* route; routes lacking a spec are
...
Two transports, mapped onto the two run targets (section 8):
...
2. **Harvest** -- for routers that expose their tree (chi `Walk`/`Routes()`,
   ...
   **Deferred** -- not built until the package is in its own repo and there is a
   real second consumer to prove its shape (see section 11).
...
Module conversion happens after extraction (section 11), not in phase 1.
